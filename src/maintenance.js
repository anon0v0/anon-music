(() => {
  ...
  async function goToTarget() {
    if (isShell && window.__TAURI__?.core?.invoke) {
      try {
        await window.__TAURI__.core.invoke('open_music', { url: target });
        return;
      } catch (_) {}
    }
    try {
  ...
  async function probe({ navigate = false, quiet = false } = {}) {
    ...
    try {
      ...
      if (navigate || consecutiveSuccess >= 2) window.setTimeout(goToTarget, 420);
      return true;
    } catch (err) {
  ...
  // Tauri 本地维护页保留窗口控制，不会出现“无边框窗口无法关闭”。
  if (isShell && !embedded) {
  ...
  // 离线小游戏：Pulse Runner。空格/↑/点击跳跃，完全本地，无外部资源。
  if (canvas) {
    ...
    function update(dt) {
      ...
      if (spawn <= 0) {
        ...
        obstacles.push({ x: W + 15, y: H - 36 - h, w: 18 + Math.random() * 17, h, glow: Math.random() });
    ...
    const activate = (e) => { if (e && ['INPUT', 'TEXTAREA', 'BUTTON'].includes(e.target?.tagName)) return; e?.preventDefault?.(); jump(); };
