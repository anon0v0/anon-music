...
class NeteaseMiniPlayer {
    ...
    constructor(element) {
        ...
        this.audio.referrerPolicy = 'no-referrer';
        ...
        this.cache = new Map();
        // 音质（持久化，供整页播放器/全屏歌词切换）
        this.quality = (typeof localStorage !== 'undefined' && localStorage.getItem('player_quality')) || 'standard';
        // 轻量事件系统（供全屏歌词 nowplaying.js 订阅）
        this._listeners = {};
        this.init();
        this.playMode = 'list';
        this.shuffleHistory = [];
    ...
    setupAudioEvents() {
        ...
        this.audio.addEventListener('playing', () => {
            // 切歌淡入：新歌真正开始出声时，从静音渐入到目标音量（继续播放/seek 不触发，flag 已消费）
            if (this._fadeIn) {
                this._fadeIn = false;
                this._fadeVolume(this.volume, 420);
            }
        });
        ...
        this.audio.addEventListener('error', async (e) => {
            // ===== 回退安全网（按要求先注释；直连播放出问题时取消注释本段，即恢复“直链播放失败→自动走服务器代理重试一次”）=====
            // const src = this.audio.src || '';
            // if (/^https?:\/\//i.test(src) && src.indexOf('/api/proxy_stream') === -1 && !this._proxyTried) {
            //     this._proxyTried = true;
            //     const proxied = '/api/proxy_stream?url=' + encodeURIComponent(src);
            //     console.warn('直链播放失败，回退服务器代理:', src);
            //     if (this.currentSong) this.currentSong.url = proxied;
    ...
    _setupMediaSession() {
        ...
        try {
            ...
            ms.setActionHandler('previoustrack', () => this.previousSong());
            ms.setActionHandler('nexttrack', () => this.nextSong());
            ms.setActionHandler('seekto', (d) => {
                if (d && typeof d.seekTime === 'number' && isFinite(this.audio.duration)) {
            ...
            ms.setActionHandler('seekbackward', (d) => {
                ...
                this.audio.currentTime = Math.max(0, this.audio.currentTime - off);
            });
            ms.setActionHandler('seekforward', (d) => {
                const off = (d && d.seekOffset) || 10;
                this.audio.currentTime = Math.min(this.audio.duration || 0, this.audio.currentTime + off);
            });
        } catch (e) { /* 部分动作浏览器不支持，忽略 */ }
    }
    /**
     * 更新媒体会话的歌曲元数据（标题/歌手/专辑/封面）——显示在锁屏与通知栏。
     * @param {SongInfo} song
     */
    _updateMediaSession(song) {
        if (!('mediaSession' in navigator) || !song) return;
        try {
            const art = song.picUrl || '/static/icons/icon-512.png';
            navigator.mediaSession.metadata = new MediaMetadata({
                title: song.name || '未知歌曲',
                artist: song.artists || '',
                album: song.album || 'Anon Music',
                artwork: [
    ...
    async playQQMusicPlaylist(playlist, startIndex = 0) {
        try {
            ...
            await this.loadCurrentSong();
            await this.play();
            if (this.config.defaultMinimized && !this.isMinimized && this.config.position !== 'static') {
                this.toggleMinimize();
            }
        } catch(e) {
            console.error('播放列表发生错误:', e);
            if (window.appNotice) window.appNotice('播放列表加载失败，请稍后重试或切换音源', 'error', 4200);
        }
    }

    /**
     * 音量线性渐变到 target(0~1)，历时 ms 毫秒，返回 Promise。用于切歌淡入淡出。
     * 单 audio 元素无法做真正的重叠交叉淡化，这里实现“换源前淡出旧歌 → 换源 → 新歌淡入”。
     */
    _fadeVolume(target, ms) {
        return new Promise((resolve) => {
            const a = this.audio;
            if (this._fadeTimer) { clearInterval(this._fadeTimer); this._fadeTimer = null; }
            target = Math.max(0, Math.min(1, target));
            const from = a.volume;
            if (!(ms > 0) || Math.abs(from - target) < 0.001) { try { a.volume = target; } catch (_) {} return resolve(); }
            const steps = Math.max(1, Math.round(ms / 25));
            let i = 0;
            this._fadeTimer = setInterval(() => {
                i++;
                const v = from + (target - from) * (i / steps);
                try { a.volume = Math.max(0, Math.min(1, v)); } catch (_) {}
                if (i >= steps) { clearInterval(this._fadeTimer); this._fadeTimer = null; resolve(); }
            }, 25);
        });
    }
    ...
    async loadCurrentSong() {
        try {
            ...
            // 切歌淡出：换源前把旧歌音量快速淡出（直接换 src 会瞬间掐断声音，必须在换源前淡出）
            if (!this.audio.paused && this.audio.volume > 0.001) {
                await this._fadeVolume(0, 200);
            }
            await this.loadSongUrl(song.id);
            // 新歌从静音起，待 'playing' 事件真正出声时淡入到目标音量
            try { this.audio.volume = 0; } catch (_) {}
            this._fadeIn = true;
            // 总是加载歌词（全屏歌词需要），即使迷你播放器未显示歌词
            await this.loadLyrics(song.id);
            this._emit('lyricsloaded', this.lyrics);
        } catch(e) {
            console.error('加载当前歌曲错误:', e);
            if (window.appNotice) window.appNotice('当前歌曲加载失败，可能受版权或音质限制', 'error', 4200);
        }
    }
    /**
     * 更新标题与艺术家信息
    ...
    async loadSongUrl(songId) {
        this._proxyTried = false;   // 每次新载入重置：允许“直链失败→回退服务器代理”各发生一次
        // 本地音乐：用 LocalMusic 的 blob URL 直接播（不请求 /api/song_url）。
        // 优先用当前会话的 blob（getUrl）——队列恢复的旧 blob 会失效，getUrl 返回本会话重建的有效 URL。
        const sid = String(songId || (this.currentSong && this.currentSong.id) || '');
        if (sid.indexOf('local:') === 0) {
            // 队列恢复的本地歌 blob 已失效（且 toEngine 清了 url）——先确保 LocalMusic 从 IndexedDB
            // 载入完成（重建本会话 objectURL），再取 url，避免冷启动竞态误判"文件已失效"
            if (window.LocalMusic && window.LocalMusic.ensureLoaded) {
                try { await window.LocalMusic.ensureLoaded(); } catch (e) {}
            }
            let u = (window.LocalMusic && window.LocalMusic.getUrl(sid.slice(6)));
            if (!u && this.currentSong) u = this.currentSong._localUrl || this.currentSong.url;
            if (u) {
                if (this.currentSong) { this.currentSong.url = u; this.currentSong._localUrl = u; }
                this.audio.src = u;
                return;
            }
            throw new Error('本地音乐文件已失效，请重新导入');
        }
        ...
        throw new Error('无法获取播放地址: QQ音乐API返回空或失败');
    }
    /* ============ 事件系统 + 对外状态/控制（供 nowplaying.js 全屏歌词） ============ */
    on(evt, cb) {
        (this._listeners[evt] = this._listeners[evt] || []).push(cb);
        return this;
    }
    off(evt, cb) {
        if (this._listeners[evt]) this._listeners[evt] = this._listeners[evt].filter(f => f !== cb);
        return this;
    }
    _emit(evt, data) {
        (this._listeners[evt] || []).forEach(cb => { try { cb(data); } catch (e) { console.error('listener err', e); } });
    }
    /** 当前完整状态快照 */
    getState() {
        return {
            ...
            duration: this.duration || this.audio.duration || 0,
    ...
    async setQuality(q) {
        ...
        this._qSwitch = true;   // 标记：切音质重载期间不让歌词回滚到顶部
        this.currentSong.url = null;
        if (this.playlist[this.currentIndex]) this.playlist[this.currentIndex].url = null;
        await this.loadSongUrl(this.currentSong.id);
        const resume = () => {
            try { this.audio.currentTime = t; } catch (e) {}
            if (wasPlaying) this.audio.play().catch(() => {});
            setTimeout(() => { this._qSwitch = false; }, 250);
        };
    ...
    parseLyrics(lyricData) {
        ...
        // 解析网易云逐字歌词 (YRC): 每行 [start,dur](wStart,wDur,0)字(wStart,wDur,0)字...
        if (lyricData.yrc && lyricData.yrc.lyric) {
            const lines = lyricData.yrc.lyric.split('\n');
            const wordRe = /\((\d+),(\d+),\d+\)([^()\n]*)/g;
            lines.forEach(raw => {
                const line = raw.trim();
                if (!line || line[0] !== '[') return;           // 跳过 {"t":..} 元数据行
                const head = line.match(/^\[(\d+),(\d+)\]/);
                if (!head) return;
                const words = []; let plain = ''; let wm;
                wordRe.lastIndex = 0;
                while ((wm = wordRe.exec(line)) !== null) {
                    const txt = wm[3];
                    if (txt === '') continue;
                    words.push({ text: txt, time: parseInt(wm[1]) / 1000, duration: parseInt(wm[2]) / 1000 });
                    plain += txt;
                }
                if (plain.trim()) {
                    const t = words.length ? words[0].time : parseInt(head[1]) / 1000;
                    lrcMap.set(t, { text: plain, words });
                }
            });
        }

        // 解析逐字歌词 (QRC)
        if (lrcMap.size === 0 && lyricData.qrc && lyricData.qrc.lyric) {
            const regex = /\[(\d+),(\d+)\](.*?)(?=\s*\[\d+,\d+\]|$)/g;
            let match;
            while ((match = regex.exec(lyricData.qrc.lyric)) !== null) {
                ...
                while ((wordMatch = wordRegex.exec(textContent)) !== null) {
                    words.push({
                        text: wordMatch[1],
                        time: parseInt(wordMatch[2]) / 1000,
                        duration: parseInt(wordMatch[3]) / 1000
                    });
                    plainText += wordMatch[1];
                }
                if (plainText) {
                    lrcMap.set(time, { text: plainText, duration, words });
                }
            }
        }
        ...
        // 合并译文：以原文(lrc/qrc/yrc)时间轴为准，译文按最近时间匹配(容差 0.6s)。
        // 不能把 tlyric 的时间戳当独立行——译文时间戳常与原文差几十 ms，那样会拆成两行、
        // 且译文行时间略早会排到原文前面，表现为"中文译文显示在外语原文上面"(用户实测 bug)。
        const lrcTimes = Array.from(lrcMap.keys()).sort((a, b) => a - b);
        const tEntries = Array.from(tlyricMap.entries()).sort((a, b) => a[0] - b[0]);
        const nearestTrans = (time) => {
            if (tlyricMap.has(time)) return tlyricMap.get(time);
            let best = '', bd = 0.6;
            for (const [tt, tx] of tEntries) { const df = Math.abs(tt - time); if (df < bd) { bd = df; best = tx; } }
            return best;
        };
        if (lrcTimes.length) {
            this.lyrics = lrcTimes.map(time => {
                const d = lrcMap.get(time);
                return { time, text: d.text, words: d.words, duration: d.duration, translation: nearestTrans(time) };
            });
        } else {
            // 极罕见：只有译文没有原文 → 直接按译文时间轴
            this.lyrics = tEntries.map(([time, text]) => ({ time, text, translation: '' }));
        }
    ...
    async previousSong() {
        ...
        await this.loadCurrentSong();
        // 主动切歌 = 明确的播放意图：即使当前是暂停状态也直接开始播放新歌
        await this.play();
    }
    /**
    ...
    _startQrcLoop() {
        const loop = () => {
            if (this.isPlaying && this.lyrics && this.currentLyricIndex >= 0 && this.lyrics[this.currentLyricIndex]?.words) {
                const words = this.lyrics[this.currentLyricIndex].words;
                const currentTime = this.audio.currentTime;
                const wordElements = this.elements.lyricLine.querySelectorAll('.qrc-word');
                if (wordElements.length === words.length) {
                    words.forEach((w, i) => {
                        ...
                        if (currentTime < w.time) {
                            percent = 0;
                        } else if (currentTime >= w.time + w.duration) {
                            percent = 100;
                        } else {
                            percent = ((currentTime - w.time) / w.duration) * 100;
                        }
                        el.style.setProperty('--qrc-fill', `${percent}%`);
                    });
                }
            }
        ...
        if (newIndex !== this.currentLyricIndex) {
            ...
            if (newIndex >= 0 && newIndex < this.lyrics.length) {
                ...
                if (lyric.words) {
                    ...
                    html = lyric.words.map(w => `<span class="qrc-word" style="--qrc-fill: 0%;">${w.text}</span>`).join('');
                } else {
                    this.elements.lyricLine.classList.remove('word-lyric');
                    html = lyricText;
                }
     ...
     */
    seekTo(e) {
        // 兼容两种调用：传入秒数(number) 或 鼠标事件(从内置进度条点击)
        if (typeof e === 'number') {
            if (isFinite(e) && e >= 0) { try { this.audio.currentTime = e; } catch (_) {} }
            return;
        }
